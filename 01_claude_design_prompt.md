# Claude Design Prompt — "Verify" Eligibility App
...
**E. System failures must never look like coverage failures.** A timeout, a malformed response, or a payer rejection has its own visual treatment that is clearly distinct from "Not Eligible." Different icon, different language, different action button. This is non-negotiable.
...
**Lucide icons** throughout. Common ones: `bell`, `circle-alert`, `message-square`, `chevron-right`, `camera`, `paperclip`, `search`, `filter`, `check-circle`, `x-circle`, `clock`, `wifi-off`, `refresh-cw`, `eye`, `eye-off`, `home`, `clock` (history), `user`. Use 20px in body, 24px in nav, 16px in badges.
...
- Header: patient name + payer + result badge.
...
- Result reveal: 400ms ease-out, with a 80ms stagger on the key-facts list items.
- Bottom-sheet open/close: 280ms ease-out spring.
...
- Sharing results externally (email/text/CRM export).
- Voice entry.
...
- Same flow but with Frank Sullivan → System error (timeout) result with Retry.
